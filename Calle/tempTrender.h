	...
	void tempOnDay(int monthToCalculate, int dayToCalculate, double specTemp = std::numeric_limits<double>::quiet_NaN()) {
		if(pathToFile_.substr(pathToFile_.length() - 4) == ".dat" || pathToFile_.substr(pathToFile_.length() - 4) == ".csv"){
			...
			if (file.is_open()) {
				...
				if(pathToFile_.substr(pathToFile_.length() - 4) == ".dat"){
				...
				else if(pathToFile_.substr(pathToFile_.length() - 4) == ".csv"){
				...
				if(specTemp == specTemp){
					...
					if(specTemp == 1){
						cout << "Probability of temp being " << specTemp << " degree on " << dayToCalculate << "/" << monthToCalculate << " is :" << tempProb*100 << " %" << endl;
					}
					else {
						cout << "Probability of temp being " << specTemp << " degrees " << dayToCalculate << "/" << monthToCalculate << " is :" << tempProb*100 << " %" << endl;
					}
				}
				...
				cout << "Mean temp on " << dayToCalculate << "/" << monthToCalculate << " is :" << mean << endl;
